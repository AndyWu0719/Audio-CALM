...
def load_calm_model(args, device):
    """
    重组训练好的 Energy-based Audio-CALM 模型
    """
    print(f"Loading Base Qwen from {args.qwen_path}...")
    
    # [CHANGE] 使用 Energy 相关的配置参数
    config = QwenCALMConfig(
        qwen_path=args.qwen_path,
        vae_path=args.vae_path,
        latent_dim=64,
        noise_size=64,       # Energy Head 参数
        num_mlp_layers=2,    # Energy Head 参数
    ...
    if os.path.exists(os.path.join(args.checkpoint, "adapter_model.bin")):
        ...
        print("No LoRA adapter found (or loaded automatically by HF).")
    ...
    if os.path.exists(output_head_path):
        # [CHANGE] Energy Head 的参数名可能略有不同，但 load_state_dict 会自动匹配
    ...
    # 3. 精度管理
    # 强制 LLM 使用 FP16 (因为 4090 上 BF16 可能导致 FlashAttn 报错，FP16 比较稳)
    dtype = torch.float16 
    model.llm.to(dtype)
    model.input_proj.to(dtype)
    model.output_head.to(dtype)
    
    # VAE 保持 float32 以确保音频重构质量
...
def generate_audio_from_text(model, tokenizer, text, max_len=256, temperature=1.0, device="cuda"):
    """
    TTS 推理：文本 -> Energy Head 采样 -> 音频 Latent
    """
    ...
    # 获取 input_proj 的权重类型
    target_dtype = model.input_proj.weight.dtype
    ...
    with torch.no_grad():
        for _ in range(max_len):
            ...
            # 2. 获取最后一个 token 的 hidden state
            last_hidden = outputs.last_hidden_state[:, -1:, :] 
            
            # 3. [CHANGE] Energy Head 采样
            # 这是一个隐式生成过程：Input(Hidden) + Noise -> Latent
            z_next = model.output_head.sample(last_hidden, temperature=temperature) # [1, 1, 64]
            
            generated_latents.append(z_next)
            
            # 4. 准备下一步输入 (Latent -> LLM Dim)
            # [CRITICAL FIX] 针对 RTX 4090 的 FP16 Linear Bug 再次手动防护
            # 虽然这里只是一步推理，但为了防止报错，先把 z_next 转为 FP32
            z_next_fp32 = z_next.to(torch.float32)
            
            # 手动调用 FP32 Linear (利用 input_proj 的权重)
            proj_weight = model.input_proj.weight.to(torch.float32)
            proj_bias = model.input_proj.bias.to(torch.float32) if model.input_proj.bias is not None else None
            
            next_embeds = F.linear(z_next_fp32, proj_weight, proj_bias)
            
            # 转回 LLM 精度 (FP16)
            curr_inputs = next_embeds.to(target_dtype)
...
def transcribe_audio(model, tokenizer, mel, device="cuda"):
    ...
    # VAE 需要 float32
    mel = mel.to(device).unsqueeze(0).to(torch.float32) 
    
    with torch.no_grad():
        mu, _ = model.vae.encode(mel)
        # [FIX] 确保 contiguous，防止 FP16 出错
        latents = mu.transpose(1, 2).contiguous() # [1, T_lat, 64]
        
        # [CRITICAL FIX] 针对 4090 的 FP16 Projection 防护
        latents_fp32 = latents.to(torch.float32)
        proj_weight = model.input_proj.weight.to(torch.float32)
        proj_bias = model.input_proj.bias.to(torch.float32) if model.input_proj.bias is not None else None
        
        audio_embeds = F.linear(latents_fp32, proj_weight, proj_bias)
        audio_embeds = audio_embeds.to(model.llm.dtype)
...
def mel_to_wav(vae, latents, device):
    ...
    try:
        ...
        wav = torch.zeros(1, 16000).to(device)
...
def main():
    ...
    parser.add_argument("--num_samples", type=int, default=8, help="Number of samples for model config (not used in inference)")
    ...
    print(f"Starting Evaluation on {len(data)} samples with Temperature={args.temperature}...")
    ...
    for i, item in enumerate(tqdm(data)):
        ...
        # [CHANGE] 传入 temperature
        gen_latents = generate_audio_from_text(model, tokenizer, text_gt, max_len=target_len, temperature=args.temperature, device=device)
