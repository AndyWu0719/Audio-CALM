...
from utils.qwen_generation_utils import (
    HistoryType,
    make_context,
    decode_tokens,
    get_stop_words_ids,
...
class QWenModel(QWenPreTrainedModel):
    ...
    def forward(
        ...
        if past_key_values is None and input_ids is not None and torch.any(input_ids == self.config.audio['audio_start_id']):
            ...
            # 1. 原始 AudioEncoder 编码 (Wav -> 4096)
            # 注意：这里 audios 出来是 [Batch, Seq_Len, 4096]
            if isinstance(audio_info, Dict):
                ...
                if input_ids is not None:
                    ...
                    input_audio_lengths = input_audio_lengths.to(device)
                
                # 获取 AudioEncoder 输出 (Batch, Seq, Dim)
                # 注意：这里我们不再手动计算 cnn_lengths，直接调用 encode，假设它内部处理正确
                # 如果需要之前的 cnn_span_tokens 逻辑，请保留，但 VAE 部分必须删除
                cnn_lengths = input_audio_lengths[:, 0].cpu().tolist()
                encoder_output_lengths = [(l - 2) // 2 + 1 for l in cnn_lengths]
                cnn_span_tokens = [l + 2 for l in encoder_output_lengths]
                
                audios = self.audio.encode(audios, input_audio_lengths, cnn_span_tokens)
        ...
        # [修改] 使用 clone() 创建副本，避免对叶子节点进行 in-place 修改
        # 开启 enable_input_require_grads() 后，inputs_embeds 视为叶子节点，直接修改会报错
        ...
        if audios is not None:
            for idx, (i, a, b) in enumerate(audio_pos):
                # [修改前] 覆盖整个 span (包括 start/end tag)，导致长度不匹配
                # hidden_states[i][a : b+1] = audios[idx]
                
                # [修改后] 只覆盖中间的 pad 部分 (a+1 到 b)，保留 start/end tag 的语义
                # audio_span_tokens = N + 2
                # input_ids: [Start(a), Pad, Pad, ..., End(b)]
                # Slice a+1 : b 的长度正好是 N，与 VAE 输出一致
                
                # 增加一个安全检查，防止 VAE 输出长度与 Tokenizer 计算有细微偏差
                target_shape = hidden_states[i][a+1 : b].shape
                source_shape = audios[idx].shape
                
                if target_shape[0] != source_shape[0]:
                    # 如果长度不一致 (通常是因为 VAE 卷积 padding 导致的 1-2 token 误差)
                    # 进行简单的插值缩放以匹配长度
                    logger.warning_once(f"Audio shape mismatch: target {target_shape[0]}, source {source_shape[0]}. Resizing...")
                    source_tensor = audios[idx].unsqueeze(0).transpose(1, 2) # [1, Dim, SrcLen]
                    source_tensor = F.interpolate(source_tensor, size=target_shape[0], mode='linear', align_corners=False)
                    audios[idx] = source_tensor.transpose(1, 2).squeeze(0) # [TgtLen, Dim]

                hidden_states[i][a+1 : b] = audios[idx]
