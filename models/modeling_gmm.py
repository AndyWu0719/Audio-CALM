...
def sample_from_gmm(pi_logits, mu, log_sigma):
    """
    从 GMM 分布中采样
    pi_logits: [B, S, K]
    mu: [B, S, K, D]
    ...
    """
    # 1. 采样混合组件索引 k
    # Categorical 接受 logits，会自动做 softmax
    dist_k = torch.distributions.Categorical(logits=pi_logits)
    k = dist_k.sample() # [B, S]

    # 2. 根据 k 收集对应的 mu 和 sigma
    # k: [B, S] -> [B, S, 1, D] 以便 gather
    k_expanded = k.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, 1, mu.size(-1))
    
    mu_k = torch.gather(mu, 2, k_expanded).squeeze(2) # [B, S, D]
    log_sigma_k = torch.gather(log_sigma, 2, k_expanded).squeeze(2) # [B, S, D]
