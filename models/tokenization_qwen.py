...
from process_data.audio import *
    ...
    def process_audio(self, text):
        ...
        if len(audio_urls) > 0:
            ...
            for audio_path in audio_urls:
                ...
                # === 修改开始 ===
                # 你的 VAE Patch Size (假设是 4)
                encoder_out_len = (audio_len_after_cnn - 2) // 2 + 1
                VAE_PATCH_SIZE = 4 
                
                # 计算 VAE 压缩后的 Token 数
                # 逻辑必须和 modeling_audioautoencoder.py 里的 Encoder 逻辑一致
                # Encoder 逻辑: seq_length // patch_size
                # [FIX] 改为 Padding 逻辑: (L+5)//4
                audio_token_num = math.ceil(encoder_out_len / VAE_PATCH_SIZE)
                
                # 确保至少有 1 个 token
                if audio_token_num < 1: audio_token_num = 1
                # === 修改结束 ===
