    ...
    for f_path in iterator:
        try:
            ...
            if mel.dim() == 3 and mel.shape[0] == 1:
                mel = mel.squeeze(0)
            assert mel.dim() == 2, f"Unexpected mel shape: {tuple(mel.shape)}"

            mel = mel.to(torch.float32)

            with torch.no_grad():
                mu, logvar = vae.encode(mel.unsqueeze(0))  # [1, 64, T_lat]
                if args.latent_type == "mu":
                    latent = mu
                else:
                ...
                latent = latent.squeeze(0).contiguous().cpu()  # [64, T_lat]
...
def main():
    ...
    parser.add_argument("--latent_type", type=str, default="mu", choices=["mu", "z"])
    ...
    print(f"Launching {num_processes} processes on {num_gpus} GPUs...")
    ...
    for rank in range(num_processes):
        ...
        if rank < len(file_chunks):
            p = mp.Process(target=process_files, args=(rank, gpu_id, file_chunks[rank], args))
            p.start()
            processes.append(p)
